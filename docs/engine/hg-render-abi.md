...
Read-only **UTF-8 SVG** page preview on the same `hg_engine*`. **PNG / native-skia forbidden.** Save stays `hg_save` / `hg_save_hwpx` clear-before-save. App Views wiring is **개발자2** (`NativePageRaster.previewProvider` → this document's `hg_engine*`). PNG / native-skia stay out.
...
| Kit | Header copy + Linux C stub. `RealEngine.renderPageSvg` + `takeBuffer`/`hg_free_buffer`. App: `KitRealEngine` / `NativePageRaster` same-session wire |
