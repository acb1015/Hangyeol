...
│       └── (fallback) StructuredTextView  # Mock / cannot preview / open failure
...
- Real + `canRenderPagePreview` → `RenderHostView(host:)` + NativePage (제품 기본 UX)
- Mock / cannot preview / open failure → `StructuredTextView`
