...
| 3 | `.hwp` 더블클릭 | **번들에 `.hwp` 샘플 없음** (`Sample/welcome.hwp` 삭제). COMMIT_OK `.hwp`도 없음 (F13). `welcome.mock.json`을 `.hwp`로 바꿔 쓰지 말 것. HOLD F11 금지. 있으면 Hangul F01만. 이 칸은 HWP 픽스처가 올 때까지 skip 가능 | ☐ |
