...
호스트 구현(PR **#50**): [native-page-host-phase1.md](native-page-host-phase1.md) · `Render/NativePageRenderHost`. 기본 UX: Real preview → NativePage.
...
| 렌더 본문 | studio-host 웹 에디터 + Tauri webview | **Path B** `#50` `Render/NativePageRenderHost` + `NativePageHostView`. 제품 미리보기는 **이 문서 세션**의 `hg_render_page_svg` (`NativePageRaster`). Real + `canRenderPagePreview` → NativePage. Mock/fail → `StructuredTextView` / placeholder. WKWebView / PNG / skia 금지 | **DONE** (default UX) |
