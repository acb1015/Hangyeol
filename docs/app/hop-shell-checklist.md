...
| 렌더 본문 | studio-host 웹 에디터 + Tauri webview | **Path B** `NativePageRaster` → `hg_render_page_svg` (같은 `DocumentSession` / `hg_engine*`). Mock→placeholder. `showsRenderHostSketch` 기본 false. PNG/skia 없음 | **PARTIAL** (SVG wire; 플래그 off) |
...
1. `showsRenderHostSketch` on 스모크 (기본은 **false** 유지). PNG / native-skia 없음
