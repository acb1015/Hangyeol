...
| 렌더 본문 | studio-host 웹 에디터 + Tauri webview | **Path B** 네이티브 페이지 호스트. 프로토콜 정본 PR **#48** `Views/HangyeolRenderHosting.swift`. WKWebView 금지. 호스트 구현은 별 PR | **TODO** |
...
1. Path B 네이티브 페이지 호스트 구현 — `#48` `HangyeolRenderHosting` (엔진 래스터는 개발자1 / [renderer-spike-1pager.md](../engine/renderer-spike-1pager.md))
