...
- **Renderer**: 제품 C ABI는 **UTF-8 SVG 페이지 미리보기만** (`hg_render_page_svg` → layer + `RenderProfile::Screen`). PNG / native-skia FFI 없음. 같은 `hg_engine*` 읽기; 저장은 여전히 `hg_save` / `hg_save_hwpx` clear-before-save. 범위 밖 페이지 → `HG_CORRUPT`. 앱은 `NativePageRaster`가 **그 창의** `DocumentSession`에서만 호출한다 (`EngineClient.current` 금지). Mock/실패는 placeholder. `showsRenderHostSketch` 기본 false. ABI: [engine/hg-render-abi.md](engine/hg-render-abi.md). 셸: [app/native-page-host-phase1.md](app/native-page-host-phase1.md).
