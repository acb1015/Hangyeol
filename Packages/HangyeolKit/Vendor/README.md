...
See `Packages/HangyeolKit/README.md`. Mac 재현 절차 (PR #14): `docs/engine/xcframework.md` 절 **로컬 재현 (2026-09-10)**. `hg_*` 심볼이 바뀌면 [docs/engine/vendor-rebuild.md](../../../docs/engine/vendor-rebuild.md) — **바이너리는 커밋하지 않는다.** `nm` 확인에 `hg_insert_text` / `hg_delete_range` / `hg_list_tables` / `hg_set_cell_text` / `hg_list_images` / `hg_render_page_svg` 가 포함돼야 한다. PNG / native-skia 심볼은 없어야 한다.
